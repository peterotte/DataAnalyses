...
//	for (Int_t k=0; k<NTaggerEnergies; k++) { //2nd step
	for (Int_t k=199; k<NTaggerEnergies; k++) { //2nd step
...
void GenerateRootFile() {
	...
	for (Int_t k=0;k<352;k++) {
		...
		while (!feof(readFile)) {
			if (!fgets(Line, sizeof(Line), readFile)) break;
			if (sscanf(Line, "%f %f %f %f %f %f %f %f %f %f", &TempValues[0], &TempValues[1], &TempValues[2], &TempValues[3], &TempValues[4], &TempValues[5], &TempValues[6], &TempValues[7], &TempValues[8], &TempValues[9]) == 10) {
				//TaggerEnergies[AktElementNr] = CentralEnergyMEV;
				MyTreeF[AktElementNr] = TempValues[7];
				MyTreeDSG[AktElementNr] = TempValues[2];
				MyTreeT[AktElementNr] = TempValues[3];
		...
		if (AktElementNr != NumberOfThetaBins) {
			printf("Number of Theta bins not correct in file.\n");
			exit(1);
		}
		MyMAIDTree->Fill();
