...
// /datapool/home/otte/StabilityChecks_withPID_TaggerScalers_20140808.root
...
void CheckTaggerScalersGoodness(int MyStart=0, int MyDraw = -1) {
	...
	if (hScaler3) {
		...
		hScaler3 = (TH2D*)gROOT->FindObject("hTaggerTest1");
	...
	hScaler3->SetAxisRange(0,1400,"X");
	...
	for (int i=0; i<1; i++) {
		...
		for (int m=2;m<hTime1D->GetNbinsX()-1; m++) {
			if ((hTime1D->GetBinContent(m) != 0)) {
				double Gleichheit1 = fabs(hTime1D->GetBinContent(m) - LetzterWert) / ( 1.0*hTime1D->GetBinContent(m) + 1.0*LetzterWert );
				double Gleichheit2 = fabs(hTime1D->GetBinContent(m) - hTime1D->GetBinContent(m+1)) / ( 1.0*hTime1D->GetBinContent(m) + 1.0*hTime1D->GetBinContent(m+1) );
				if ( (Gleichheit1 > (0.05/2)) && (Gleichheit2 > (0.05/2)) ) {
					if (MyDraw) printf("R %d:\t%f\t%f\n",m,Gleichheit1,Gleichheit2);
				} else {
					//printf("%d:\t%f\t%f\n",m,Gleichheit1,Gleichheit2);
