...
int ReadBaFConfigurationFile() {
    ...
    RawADCData.BaF.Elements.resize(NBaFElements);

    FILE *readFile = NULL;
    Char_t TempStr[1024];
    strcpy(TempStr, InputPathDataDir);
    strcat(TempStr, "/TAPS_BaF2.dat");
    readFile=fopen(TempStr, "r");// text Datei zum Schreiben "r" aufmachen.

    if(readFile == NULL)//Öffnen nicht geklappt, steig aus:
    {
        fprintf(stderr,"Could not open Baf config file for reading.\n");
        exit(1);
    } else {
        Printf("INFO: BaF file: %s", TempStr);
    }
    ...
    while (!feof(readFile)) {
        ...
        if (sscanf(Line, "%s %f ", Buffer, &SingleFloatToRead ) == 2) {
            if (!strcmp(Buffer, "TimeOffsetNS:")) {
                RawADCData.BaF.TimeOffsetNS = SingleFloatToRead;
                Printf("Info: BaF TimeOffsetNS: %f", RawADCData.BaF.TimeOffsetNS);
            }
        }
        ...
        if (sscanf(Line, "%s %d %f %f %f %f %s %f %f %f %f %f %f %f", Buffer, &ADCChNr, &ADCLowThresholdMEV, &ADCHighThresholdMEV, &ADCPedestalTicks, &ADCMEVPerTick, TDCChNr_Str, &TDCLowThresholdNS, &TDCHighThresholdNS, &TDCOffsetTicks,
            &TDCNSPerTick, &Position0, &Position1, &Position2 ) == 14) {
            if (!strcmp(Buffer, "Element:")) {
                //BaF have only 1 TDC Info
                //if (strlen(TDCChNr_Str)>2) {TDCChNr_Str[strlen(TDCChNr_Str)-2] = '\0'; } //Cut away the "M0" part
                if (sscanf(TDCChNr_Str, "%d", &TDCChNr) != 1) {Printf("Error reading TDC Ch.");}
                //Printf("BaF Element added: %d %f %f %f %f %d %f %f %f %f %f %f %f", ADCChNr, ADCLowThresholdMEV, ADCHighThresholdMEV, ADCPedestalTicks, ADCMEVPerTick, TDCChNr,
                //       TDCLowThresholdNS, TDCHighThresholdNS, TDCOffsetTicks, TDCNSPerTick, Position0, Position1, Position2);
                RawADCData.BaF.Elements.at(AktElementNr).ADCCh = ADCChNr;
                RawADCData.BaF.Elements.at(AktElementNr).TDCCh = TDCChNr;
                RawADCData.BaF.Elements.at(AktElementNr).ScalerCh = -1;

                RawADCData.BaF.Elements.at(AktElementNr).TDCLowThresholdNS = TDCLowThresholdNS;
                RawADCData.BaF.Elements.at(AktElementNr).TDCHighThresholdNS = TDCHighThresholdNS;
                RawADCData.BaF.Elements.at(AktElementNr).TDCOffsetTicks = TDCOffsetTicks;
                RawADCData.BaF.Elements.at(AktElementNr).TDCNSPerTick = TDCNSPerTick;

                //ADC calibration
                RawADCData.BaF.Elements.at(AktElementNr).ADCLowThresholdMEV = ADCLowThresholdMEV;
                RawADCData.BaF.Elements.at(AktElementNr).ADCHighThresholdMEV = ADCHighThresholdMEV;
                RawADCData.BaF.Elements.at(AktElementNr).ADCPedestalTicks = ADCPedestalTicks;
                RawADCData.BaF.Elements.at(AktElementNr).ADCMEVPerTick = ADCMEVPerTick;

                RawADCData.BaF.Elements.at(AktElementNr).CentralEnergyMEV = -1;

                //Position, Values are provided in x, y, z
                double TempVar = sqrt( pow(Position0,2.)+pow(Position1,2.)+pow(Position2,2.) );
                RawADCData.BaF.Elements.at(AktElementNr).Position.r = TempVar;
                RawADCData.BaF.Elements.at(AktElementNr).Position.theta = TMath::ACos(Position2/TempVar);
                RawADCData.BaF.Elements.at(AktElementNr).Position.phi = TMath::ATan2(Position1, Position0);

                if (LookupTableADC.at(ADCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", ADCChNr); } else {
                    LookupTableADC.at(ADCChNr).DetectorID = DetectorIDBaF;
                    LookupTableADC.at(ADCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(ADCChNr).TypeID = ADCTypeIDADC;
                }
                if (LookupTableADC.at(TDCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", TDCChNr); } else {
                    LookupTableADC.at(TDCChNr).DetectorID = DetectorIDBaF;
                    LookupTableADC.at(TDCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(TDCChNr).TypeID = ADCTypeIDTDC;
                }
                AktElementNr++;
            } else {
                //Printf("Error reading file: Line does not start with Element:");
    ...
    if (AktElementNr != NBaFElements) {
        Printf("ERROR: BaF: Read only %d elements, %d elements expected.", AktElementNr, NBaFElements);
        exit(1);
    } else {
        Printf("INFO: BaF: Read %d elements.", AktElementNr);
    }
...
int ReadTAPSVetoConfigurationFile() {
    Printf("--------------------------------------");
    Char_t Line[1024];
    Char_t Buffer[1024];
    int ADCChNr = -1;
    ...
    RawADCData.TAPSVeto.Elements.resize(NTAPSVetoElements);

    FILE *readFile = NULL;
    Char_t TempStr[1024];
    strcpy(TempStr, InputPathDataDir);
    strcat(TempStr, "/TAPS_Veto.dat");
    readFile=fopen(TempStr, "r");// text Datei zum Schreiben "r" aufmachen.

    if(readFile == NULL)//Öffnen nicht geklappt, steig aus:
    {
        fprintf(stderr,"Could not open TAPSVeto config file for reading.\n");
        exit(1);
    } else {
        Printf("INFO: TAPSVeto file: %s", TempStr);
    }
    ...
    while (!feof(readFile)) {
        ...
        if (sscanf(Line, "%s %f ", Buffer, &SingleFloatToRead ) == 2) {
            if (!strcmp(Buffer, "TimeOffsetNS:")) {
                RawADCData.TAPSVeto.TimeOffsetNS = SingleFloatToRead;
                Printf("Info: TAPSVeto TimeOffsetNS: %f", RawADCData.TAPSVeto.TimeOffsetNS);
            }
        }
        ...
        if (sscanf(Line, "%s %d %f %f %f %f %s %f %f %f %f %f %f %f", Buffer, &ADCChNr, &ADCLowThresholdMEV, &ADCHighThresholdMEV, &ADCPedestalTicks, &ADCMEVPerTick, TDCChNr_Str, &TDCLowThresholdNS, &TDCHighThresholdNS, &TDCOffsetTicks,
            &TDCNSPerTick, &Position0, &Position1, &Position2 ) == 14) {
            if (!strcmp(Buffer, "Element:")) {
                //TAPSVeto have only 1 TDC Info
                //if (strlen(TDCChNr_Str)>2) {TDCChNr_Str[strlen(TDCChNr_Str)-2] = '\0'; } //Cut away the "M0" part
                if (sscanf(TDCChNr_Str, "%d", &TDCChNr) != 1) {Printf("Error reading TDC Ch.");}
                //Printf("TAPSVeto Element added: %d %f %f %f %f %d %f %f %f %f %f %f %f", ADCChNr, ADCLowThresholdMEV, ADCHighThresholdMEV, ADCPedestalTicks, ADCMEVPerTick, TDCChNr,
                //       TDCLowThresholdNS, TDCHighThresholdNS, TDCOffsetTicks, TDCNSPerTick, Position0, Position1, Position2);
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ADCCh = ADCChNr;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).TDCCh = TDCChNr;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ScalerCh = -1;

                RawADCData.TAPSVeto.Elements.at(AktElementNr).TDCLowThresholdNS = TDCLowThresholdNS;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).TDCHighThresholdNS = TDCHighThresholdNS;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).TDCOffsetTicks = TDCOffsetTicks;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).TDCNSPerTick = TDCNSPerTick;

                //ADC calibration
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ADCLowThresholdMEV = ADCLowThresholdMEV;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ADCHighThresholdMEV = ADCHighThresholdMEV;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ADCPedestalTicks = ADCPedestalTicks;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).ADCMEVPerTick = ADCMEVPerTick;

                RawADCData.TAPSVeto.Elements.at(AktElementNr).CentralEnergyMEV = -1;

                //Position, Values are provided in x, y, z
                double TempVar = sqrt( pow(Position0,2.)+pow(Position1,2.)+pow(Position2,2.) );
                RawADCData.TAPSVeto.Elements.at(AktElementNr).Position.r = TempVar;
                RawADCData.TAPSVeto.Elements.at(AktElementNr).Position.theta = TMath::ACos(Position2/TempVar);
                RawADCData.TAPSVeto.Elements.at(AktElementNr).Position.phi = TMath::ATan2(Position1, Position0);

                if (LookupTableADC.at(ADCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", ADCChNr); } else {
                    LookupTableADC.at(ADCChNr).DetectorID = DetectorIDTAPSVeto;
                    LookupTableADC.at(ADCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(ADCChNr).TypeID = ADCTypeIDADC;
                }
                if (LookupTableADC.at(TDCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", TDCChNr); } else {
                    LookupTableADC.at(TDCChNr).DetectorID = DetectorIDTAPSVeto;
                    LookupTableADC.at(TDCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(TDCChNr).TypeID = ADCTypeIDTDC;
                }
                AktElementNr++;
    ...
    if (AktElementNr != NTAPSVetoElements) {
        Printf("ERROR: TAPSVeto: Read only %d elements, %d elements expected.", AktElementNr, NTAPSVetoElements);
        exit(1);
    } else {
        Printf("INFO: TAPSVeto: Read %d elements.", AktElementNr);
    }
...
int ReadPbWOConfigurationFile() {
    ...
    RawADCData.PbWO.Elements.resize(NPbWOElements);

    FILE *readFile = NULL;
    Char_t TempStr[1024];
    strcpy(TempStr, InputPathDataDir);
    strcat(TempStr, "/TAPS_PbWO4.dat");
    readFile=fopen(TempStr, "r");// text Datei zum Schreiben "r" aufmachen.

    if(readFile == NULL)//Öffnen nicht geklappt, steig aus:
    {
        fprintf(stderr,"Could not open PbWO config file for reading.\n");
        exit(1);
    } else {
        Printf("INFO: PbWO file: %s", TempStr);
    }
    ...
    while (!feof(readFile)) {
        ...
        if (sscanf(Line, "%s %f ", Buffer, &SingleFloatToRead ) == 2) {
            if (!strcmp(Buffer, "TimeOffsetNS:")) {
                RawADCData.PbWO.TimeOffsetNS = SingleFloatToRead;
                Printf("Info: PbWO TimeOffsetNS: %f", RawADCData.PbWO.TimeOffsetNS);
            }
        }
        ...
        if (sscanf(Line, "%s %d %f %f %f %f %s %f %f %f %f %f %f %f", Buffer, &ADCChNr, &ADCLowThresholdMEV, &ADCHighThresholdMEV, &ADCPedestalTicks, &ADCMEVPerTick, TDCChNr_Str, &TDCLowThresholdNS, &TDCHighThresholdNS, &TDCOffsetTicks,
            ...
            if (!strcmp(Buffer, "Element:")) {
                if (strlen(TDCChNr_Str)>2) {TDCChNr_Str[strlen(TDCChNr_Str)-2] = '\0'; } //Cut away the "M0" part
                if (sscanf(TDCChNr_Str, "%d", &TDCChNr) != 1) {Printf("Error reading TDC Ch.");}
                //Printf("PbWO Element added: %d %f %f %f %f %d %f %f %f %f %f %f %f", ADCChNr, ADCLowThresholdMEV, ADCHighThresholdMEV, ADCPedestalTicks, ADCMEVPerTick, TDCChNr,
                //       TDCLowThresholdNS, TDCHighThresholdNS, TDCOffsetTicks, TDCNSPerTick, Position0, Position1, Position2);
                RawADCData.PbWO.Elements.at(AktElementNr).ADCCh = ADCChNr;
                RawADCData.PbWO.Elements.at(AktElementNr).TDCCh = TDCChNr;
                RawADCData.PbWO.Elements.at(AktElementNr).ScalerCh = -1;

                RawADCData.PbWO.Elements.at(AktElementNr).TDCLowThresholdNS = TDCLowThresholdNS;
                RawADCData.PbWO.Elements.at(AktElementNr).TDCHighThresholdNS = TDCHighThresholdNS;
                RawADCData.PbWO.Elements.at(AktElementNr).TDCOffsetTicks = TDCOffsetTicks;
                RawADCData.PbWO.Elements.at(AktElementNr).TDCNSPerTick = TDCNSPerTick;

                //ADC calibration
                RawADCData.PbWO.Elements.at(AktElementNr).ADCLowThresholdMEV = ADCLowThresholdMEV;
                RawADCData.PbWO.Elements.at(AktElementNr).ADCHighThresholdMEV = ADCHighThresholdMEV;
                RawADCData.PbWO.Elements.at(AktElementNr).ADCPedestalTicks = ADCPedestalTicks;
                RawADCData.PbWO.Elements.at(AktElementNr).ADCMEVPerTick = ADCMEVPerTick;

                RawADCData.PbWO.Elements.at(AktElementNr).CentralEnergyMEV = -1;

                //Position, Values are provided in x, y, z
                double TempVar = sqrt( pow(Position0,2.)+pow(Position1,2.)+pow(Position2,2.) );
                RawADCData.PbWO.Elements.at(AktElementNr).Position.r = TempVar;
                RawADCData.PbWO.Elements.at(AktElementNr).Position.theta = TMath::ACos(Position2/TempVar);
                RawADCData.PbWO.Elements.at(AktElementNr).Position.phi = TMath::ATan2(Position1, Position0);

                if (LookupTableADC.at(ADCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", ADCChNr); } else {
                    LookupTableADC.at(ADCChNr).DetectorID = DetectorIDPbWO;
                    LookupTableADC.at(ADCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(ADCChNr).TypeID = ADCTypeIDADC;
                }
                if (LookupTableADC.at(TDCChNr).DetectorID != -1) { Printf("ERROR: ADCch %d already configured.\n", TDCChNr); } else {
                    LookupTableADC.at(TDCChNr).DetectorID = DetectorIDPbWO;
                    LookupTableADC.at(TDCChNr).ElementID = AktElementNr;
                    LookupTableADC.at(TDCChNr).TypeID = ADCTypeIDTDC;
                }
                AktElementNr++;
    ...
    if (AktElementNr != NPbWOElements) {
        Printf("ERROR: PbWO: Read only %d elements, %d elements expected.", AktElementNr, NPbWOElements);
        exit(1);
    } else {
        Printf("INFO: PbWO: Read %d elements.", AktElementNr);
    }
...
int ReadConfigurationFiles() {
    ...
    ReadBaFConfigurationFile();
    ReadTAPSVetoConfigurationFile();
    ReadPbWOConfigurationFile();
}
